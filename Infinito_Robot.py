...
class FuzzyController:
    """
    Clase que implementa un controlador difuso con funciones de membresía triangular y trapezoidal.
    """
    def triangular(self, x, a, b, c):
        """
        Función de membresía triangular.
        :param x: Valor de entrada.
        :param a: Límite izquierdo del triángulo.
        :param b: Pico del triángulo.
        :param c: Límite derecho del triángulo.
        :return: Grado de membresía.
        """
        if x <= a or x >= c:
            return 0.0
        elif a < x < b:
    ...
    def trapezoidal(self, x, a, b, c, d):
        ...
        :param x: Valor de entrada.
        :param a: Límite izquierdo inferior del trapecio.
        :param b: Límite izquierdo superior del trapecio.
        :param c: Límite derecho superior del trapecio.
        :param d: Límite derecho inferior del trapecio.
        :return: Grado de membresía.
        """
        if x <= a or x >= d:
            return 0.0
    ...
    def defusificar(self, entradas_difusas, valores_singleton):
        ...
        :param entradas_difusas: Lista de grados de membresía.
        :param valores_singleton: Lista de valores singleton.
        :return: Valor defusificado.
        """
...
class DifferentialDriveSimulator:
    """
    Clase para simular un robot de movimiento diferencial en PyBullet.
    """
    def __init__(self):
        # Conexión al simulador de PyBullet
        self.physicsClient = p.connect(p.GUI)
        p.setAdditionalSearchPath(pybullet_data.getDataPath())
        p.setGravity(0, 0, -9.81)
        self.Ts = 0.005  # Tiempo de paso de simulación
        p.setTimeStep(self.Ts)
        p.setRealTimeSimulation(0)

        # Parámetros físicos del robot
        self.m = 3.0  # Masa del robot
        self.J = 0.01  # Momento de inercia
        self.L = 0.3  # Distancia entre ruedas
        self.r = 0.05  # Radio de las ruedas
        self.a = 3.0  # Parámetro de la trayectoria deseada
        self.b = 1.5  # Parámetro de la trayectoria deseada
        self.w_circ = 0.4  # Frecuencia angular de la trayectoria deseada
        self.cx = 0.0  # Posición inicial X
        self.cy = 0.0  # Posición inicial Y

        # Configuración para almacenar trayectorias
        ...
        # Inicialización del controlador difuso
        ...
        # Configuración del entorno y carga del robot
        self.setup_environment()
        self.load_robot()

    def setup_environment(self):
        """
        Configura el entorno de simulación (plano).
        """
    ...
    def create_robot_urdf(self):
        """
        Crea un archivo URDF temporal para el robot.
        """
...
<robot name="three_wheel_robot">
    ...
    <link name="chassis">
        <visual><geometry><box size="0.4 0.3 0.1"/></geometry><material name="black"/></visual>
        <collision><geometry><box size="0.4 0.3 0.1"/></geometry></collision>
        <inertial><mass value="{self.m}"/><inertia ixx="{self.J}" ixy="0" ixz="0" iyy="{self.J}" iyz="0" izz="{self.J}"/></inertial>
    </link>
    <link name="left_wheel">
        <visual><origin rpy="1.5708 0 0" xyz="0 0 0"/><geometry><cylinder radius="{self.r}" length="0.04"/></geometry></visual>
        <collision><origin rpy="1.5708 0 0" xyz="0 0 0"/><geometry><cylinder radius="{self.r}" length="0.04"/></geometry></collision>
    ...
    <joint name="left_wheel_joint" type="continuous">
        ...
        <axis xyz="0 1 0"/><origin xyz="-0.1 {self.L/2} -0.05"/>
    </joint>
    ...
    def load_robot(self):
        """
        Carga el robot desde el archivo URDF generado.
        """
        urdf_file = self.create_robot_urdf()
        initial_position = [self.cx-0.3, self.cy-0.3, 0.05]
        initial_orientation = p.getQuaternionFromEuler([0, 0, np.pi / 4])
        ...
        # Identifica las articulaciones motorizadas
        ...
        if None in [self.left_wheel_joint, self.right_wheel_joint]:
            raise Exception("No se encontraron las articulaciones motorizadas (ruedas izquierda/derecha).")
        ...
        # Elimina el archivo URDF temporal
    ...
    def reset_camera(self):
        """
        Reinicia la vista de la cámara para observar el robot.
        """
    ...
    def calculate_desired_trajectory(self, steps):
        """
        Calcula la trayectoria deseada para el robot.
        """
        t = np.arange(0, steps * self.Ts, self.Ts)
        x_d = self.cx + self.a * np.sin(self.w_circ * t)
        y_d = self.cy + self.b * np.sin(self.w_circ * t) * np.cos(self.w_circ * t)
        dx_d = self.a * self.w_circ * np.cos(self.w_circ * t)
        dy_d = self.b * self.w_circ * (np.cos(self.w_circ * t)**2 - np.sin(self.w_circ * t)**2)
    ...
    def setup_fuzzy_sets(self):
        """
        Configura los conjuntos difusos para el controlador.
        """
        self.rho_sets = {
            ...
            "MEDIO": lambda x: self.fuzzy_controller.triangular(x, 0.15, 0.25, 0.35),
            "LEJOS": lambda x: self.fuzzy_controller.triangular(x, 0.30, 0.45, 0.60),
            "MUY_LEJOS": lambda x: self.fuzzy_controller.trapezoidal(x, 0.50, 0.65, 1.00, 1.00)
        }
        self.v_singleton_values = [0.3, 1.0, 2.0, 3.0, 4.0]
        self.alpha_sets = {
            "NG": lambda x: self.fuzzy_controller.trapezoidal(x, -np.pi/2, -np.pi/2, -np.pi/3, -np.pi/4),
            "NP": lambda x: self.fuzzy_controller.triangular(x, -np.pi/3, -np.pi/4, 0),
            "ZE": lambda x: self.fuzzy_controller.triangular(x, -np.pi/8, 0, np.pi/8),
            "PP": lambda x: self.fuzzy_controller.triangular(x, 0, np.pi/4, np.pi/3),
            "PG": lambda x: self.fuzzy_controller.trapezoidal(x, np.pi/4, np.pi/3, np.pi/2, np.pi/2)
        }
        self.beta_sets = {
            "NG": lambda x: self.fuzzy_controller.trapezoidal(x, -np.pi, -np.pi, -3*np.pi/4, -np.pi/2),
            "NP": lambda x: self.fuzzy_controller.triangular(x, -3*np.pi/4, -np.pi/2, 0),
            "ZE": lambda x: self.fuzzy_controller.triangular(x, -np.pi/6, 0, np.pi/6),
            "PP": lambda x: self.fuzzy_controller.triangular(x, 0, np.pi/2, 3*np.pi/4),
            "PG": lambda x: self.fuzzy_controller.trapezoidal(x, np.pi/2, 3*np.pi/4, np.pi, np.pi)
        }
    ...
    def control_step(self, k, x_d, y_d, phi_d_target):
        """
        Realiza un paso de control del robot.
        """
    ...
    def update_visualization(self, x, y, x_d, y_d, k):
        """
        Actualiza la visualización de la trayectoria real y deseada.
        """
    ...
    def run_simulation(self, steps=30000):
        """
        Ejecuta la simulación del robot.
        """
    ...
    def plot_results(self, omega_d_plot):
        ...
        """
        plt.figure(figsize=(20, 10)) # Aumentar el tamaño de la figura para más subplots
        # Gráfica de la trayectoria (X vs Y)
        plt.subplot(2, 2, 1) # Cambiado a 2 filas, 2 columnas, posición 1
...
if __name__ == "__main__":
    ...
    simulator.run_simulation(steps=5000)