...
class DifferentialDriveSimulator:
    def __init__(self):
        ...
        # Parámetros del robot (DE TU URDF)
        self.m = 3.0            # masa total [kg]
        self.J = 0.01           # inercia alrededor del eje vertical [kg·m²]
        self.L = 0.3            # distancia entre ruedas [m]
        self.r = 0.05           # radio de las ruedas [m]

        # Trayectoria circular (personalizable)
        self.R = 2.0            # Radio del círculo
        self.w_circ = 0.6       # Velocidad angular deseada (REDUCIDA LIGERAMENTE para mayor estabilidad)
        self.cx = 0.0           # Centro X del círculo
        self.cy = 0.0           # Centro Y del círculo
...
<robot name="three_wheel_robot">
    ...
    def setup_fuzzy_sets(self):
        """Define los conjuntos difusos para las entradas y salidas, ajustados para un seguimiento óptimo."""
        
        # --- Conjuntos difusos para rho (distancia) ---
        # Hacemos los rangos más agresivos y los valores de salida más altos
        self.rho_sets = {
            "MUY_CERCANO": lambda x: self.fuzzy_controller.trapezoidal(x, -0.01, 0, 0.03, 0.08), # Rango más estrecho y cercano a cero
            ...
            "MUY_LEJOS": lambda x: self.fuzzy_controller.trapezoidal(x, 0.7, 1.0, 1.5, 1.5) # Rango más concentrado
        }
        # Valores de salida para v_ref: AUMENTADOS para una corrección más rápida
        self.v_singleton_values = [0.5, 1.5, 2.5, 3.5, 4.5] # Más velocidad para cerrar el error de distancia

        # --- Conjuntos difusos para alpha (orientación) y beta (curvatura) ---
        # Se mantienen los rangos angulares, pero se incrementan las salidas
        self.alpha_sets = {
        ...
        # Singleton para omega_ref: AUMENTAR los valores de giro para una corrección más fuerte
        self.omega_singleton_values = {
            "RG": -30.0, # Mayor magnitud para giros agresivos
            "RP": -10.0,  
            "Z":   0.0,  
            "LP":  10.0,  
            "LG": 30.0   # Mayor magnitud
        }
    ...
    def control_step(self, k, x_d, y_d, phi_d_target):
        ...
        # Look-ahead: Ajustado a un valor más conservador si la velocidad es menor
        # Si w_circ es 0.6, el robot se mueve más lento, el look-ahead puede ser un poco menor.
        look_ahead_steps = 20 
        ...
        # --- Control de Velocidad Lineal (v_ref) basado solo en rho ---
        ...
        # --- Control de Velocidad Angular (omega_ref) basado en alpha y beta ---
        ...
        # Calcular aceleraciones (usando las velocidades de referencia)
    ...
    def update_visualization(self, x, y, x_d, y_d, k):
        """Actualiza visualización de trayectorias"""
        self.real_traj.append([x, y, 0.01])
        if len(self.real_traj) > 1:
            p.addUserDebugLine(
                self.real_traj[-2],
                self.real_traj[-1],
                lineColorRGB=[1, 0, 0], # Verde para la real
                lineWidth=6,
                lifeTime=0
            )
        ...
        if k < len(x_d) and k < len(y_d):
            ...
            if len(self.desired_traj) > 1:
                p.addUserDebugLine(
                    self.desired_traj[-2],
                    self.desired_traj[-1],
                    lineColorRGB=[0, 0, 1], # Rojo para la deseada
                    lineWidth=3,
                    lifeTime=0
                )
...
if __name__ == "__main__":
    ...
    simulator.run_simulation(steps=2500)