...
<robot name="three_wheel_robot">
    ...
    def load_robot(self):
        ...
        # Posición inicial para el trébol (en x=a, y=0)
        initial_position = [self.cx + 4.0, self.cy, 0.05]
        # Orientación inicial apuntando hacia arriba (pi/2)
        initial_orientation = p.getQuaternionFromEuler([0, 0, np.pi / 2])
    ...
    def calculate_desired_trajectory(self, steps):
        ...
        # Parámetros del trébol (3 pétalos)
        a = 4.0
        n_petals = 3
        w = 0.2
        theta = w * t
        
        x_d = self.cx + a * np.cos(n_petals * theta) * np.cos(theta)
        y_d = self.cy + a * np.cos(n_petals * theta) * np.sin(theta)
        
        dx_d = -a * w * n_petals * np.sin(n_petals * theta) * np.cos(theta) - \
                a * w * np.cos(n_petals * theta) * np.sin(theta)
        dy_d = -a * w * n_petals * np.sin(n_petals * theta) * np.sin(theta) + \
                a * w * np.cos(n_petals * theta) * np.cos(theta)
